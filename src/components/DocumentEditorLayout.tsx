...
import { FileText, ChevronLeft, ChevronRight, CreditCard, CheckCircle2, Trash2, Eye, Edit3 } from 'lucide-react';
...
interface DocumentEditorLayoutProps {
  ...
  checkoutType: 'cv' | 'resume';
  buildPdfBody: (sessionId: string) => Record<string, unknown>;
...
export function DocumentEditorLayout({
  ...
  const handleDownload = useCallback(async (sid: string) => {
    setIsProcessing(true);
    try {
      const response = await fetch('/api/pdf', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(buildPdfBody(sid)),
      });
  ...
  }, [buildPdfBody, downloadFilename, redirectPath, router]);

  useEffect(() => {
    if (sessionId && !isPaid && hasHydrated) {
      handleDownload(sessionId);
    }
  }, [sessionId, isPaid, hasHydrated, handleDownload]);
  ...
  const handleCheckout = async () => {
    ...
    try {
      const response = await fetch('/api/checkout', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ email: checkoutEmail, type: checkoutType }),
      });
  ...
  const goToNext = () => currentIndex < tabs.length - 1 && setActiveTab(tabs[currentIndex + 1].id);
  const goToPrev = () => currentIndex > 0 && setActiveTab(tabs[currentIndex - 1].id);
